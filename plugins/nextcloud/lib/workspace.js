...
export async function snapshotWorkspaceFile(workspace, input, signal) {
    ...
    if (!inside(root, path))
        throw new Error('workspace file is outside session workspace');
...
function safeBasename(remotePath) {
...
export async function downloadIntoWorkspace(workspace, remotePath, input, explicitDestination, signal) {
    const root = await canonicalWorkspace(workspace);
    ...
    if (!inside(root, initial))
        throw new Error('download destination is outside session workspace');
    ...
    for (let index = 0; index < 10_000; index += 1) {
        ...
        try {
            ...
            if (error.code === 'EEXIST' && explicitDestination === undefined)
                continue;
