...
export function normalizeRemotePath(input) {
    if (typeof input !== 'string' || !input.startsWith('/') || /[\\\0]/u.test(input))
        invalidPath();
    if (/%(?:2f|5c|00)/iu.test(input))
        invalidPath();
...
export function createPathPolicy(settings) {
    ...
    return {
        ...
        assertDeletable(input) {
            const path = normalizeRemotePath(input);
            if (path === '/')
                throw new Error('cannot delete the Nextcloud file root');
            assertAllowed(path);
            if (roots.includes(path))
                throw new Error('cannot delete a configured root');
            return path;
        },
    };
}
