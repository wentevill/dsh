...
interface ApprovalBinding {
  ...
  readonly preparationError?: { readonly code: string; readonly message: string }
}
...
function cwd(exec: Readonly<ToolExecution>): string {
  const value = (exec.agent as { session?: { header?: { cwd?: string } } } | undefined)?.session?.header?.cwd
  if (value === undefined) throw new Error('session workspace is required')
  return value
}

function argsFingerprint(value: unknown): string {
...
function shareIdArg(args: Record<string, unknown>): number {
  const value = args.shareId
  if (!Number.isSafeInteger(value) || (value as number) < 1) throw new Error('shareId must be a positive integer')
...
function normalizeShareTarget(value: string): ShareCreateRequest['target'] {
  const aliases: Record<string, ShareCreateRequest['target']> = {
  ...
  return aliases[value] ?? value as ShareCreateRequest['target']
}

function normalizeShareProfile(value: string): ShareCreateRequest['profile'] {
  const aliases: Record<string, ShareCreateRequest['profile']> = {
    read: 'read', readonly: 'read', readOnly: 'read', edit: 'edit', editable: 'edit',
    fileDrop: 'fileDrop', filedrop: 'fileDrop', file_drop: 'fileDrop',
  }
  return aliases[value] ?? value as ShareCreateRequest['profile']
}

function normalizeShareeType(value: string): 'all' | 'user' | 'group' {
  const aliases: Record<string, 'all' | 'user' | 'group'> = {
    all: 'all', user: 'user', person: 'user', people: 'user',
    group: 'group', department: 'group', departments: 'group',
  }
  return aliases[value] ?? value as 'all' | 'user' | 'group'
}

function createShareArgs(args: Record<string, unknown>): ShareCreateRequest {
  return {
...
function updateShareArgs(args: Record<string, unknown>): ShareUpdateRequest {
...
function rendered(value: unknown, untrusted = false): string {
...
function failure(error: Error): { readonly ok: false; readonly error: { readonly code: string; readonly message: string } } {
...
export class NextcloudToolManager {
  ...
  private async prepareChecked(exec: Readonly<ToolExecution>): Promise<string> {
    if (!WRITE_TOOLS.has(exec.name)) throw new Error('Nextcloud tool does not require approval')
    const args = exec.arguments as Record<string, unknown>
    const snapshot = await this.resolve()
    let binding: ApprovalBinding = { name: exec.name, arguments: argsFingerprint(args), fingerprint: snapshot.fingerprint }
    let reason: string
  ...
  private async consume(exec: Readonly<ToolExecution>): Promise<ServiceSnapshot> {
    ...
    if (binding.preparationError !== undefined) throw Object.assign(new Error(binding.preparationError.message), { code: binding.preparationError.code })
    const snapshot = await this.resolve()
    if (snapshot.fingerprint !== binding.fingerprint) throw new Error('fresh approval is required because Nextcloud settings or credentials changed')
    const args = exec.arguments as Record<string, unknown>
    if (binding.localSha256 !== undefined) {
      const local = await snapshotWorkspaceFile(cwd(exec), stringArg(args, 'localPath'), exec.signal)
      if (local.sha256 !== binding.localSha256 || local.size !== binding.localSize) throw new Error('fresh approval is required because the local file changed')
    }
    if (binding.etag !== undefined) {
      const path = exec.name === 'nextcloud_move' ? stringArg(args, 'source') : stringArg(args, 'path')
      if ((await snapshot.service.stat(path, exec.signal)).etag !== binding.etag) throw new Error('fresh approval is required because the remote file changed')
    }
    if (binding.shareState !== undefined) {
      if (JSON.stringify(await snapshot.sharing.get(shareIdArg(args), exec.signal)) !== binding.shareState) throw new Error('fresh approval is required because the remote share changed')
    }
    return snapshot
  }
  ...
  private definitions(allowDelete: boolean): ToolDefinition[] {
    ...
    const executeSharingRead = (operation: (sharing: NextcloudSharingService, args: Record<string, unknown>, exec: ToolExecution) => Promise<unknown>) =>
      async (args: unknown, exec: ToolExecution) => {
        try { return rendered(await operation((await this.resolve()).sharing, args as Record<string, unknown>, exec), true) } catch (error) { return failureOutput(error) }
      }
    const executeMutation = (operation: (snapshot: ServiceSnapshot, args: Record<string, unknown>, exec: ToolExecution) => Promise<unknown>) =>
      async (args: unknown, exec: ToolExecution) => {
        try { return rendered(await operation(await this.consume(exec), args as Record<string, unknown>, exec)) } catch (error) { return failureOutput(error) }
      }
