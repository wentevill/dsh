...
function contentTypeFor(request: MailAttachmentRequest, filename: string): string {
  if (request.contentType === undefined) return lookup(filename) || 'application/octet-stream'
...
async function closeAll(
  handles: readonly { handle: FileHandle; path: string }[],
  afterClose: AttachmentLoaderHooks['afterClose'],
): Promise<void> {
