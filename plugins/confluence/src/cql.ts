...
function literal(value: string): string {
  if (value.length === 0 || value.length > 500 || /[\r\n]/u.test(value)) throw confluenceError('CQL value is invalid', 'CONFLUENCE_INPUT_INVALID')
...
export function buildPageCql(input: PageCqlInput): string {
  ...
  for (const label of input.labels) clauses.push(`label = ${literal(label.trim())}`)
  if (spaces.length > 0) clauses.push(`space IN (${spaces.map(literal).join(', ')})`)
