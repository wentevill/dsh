...
export function markdownToStorage(value: string): string {
  if (value.length > 500_000) throw new Error('confluence: Markdown exceeds 500000 characters')
  return markdown.render(value).trim().replace(/<br>/gu, '<br />').replace(/<hr>/gu, '<hr />')
}

export function storageToText(value: string, maxChars = 100_000): { text: string; truncated: boolean } {
