...
const staging = mkdtempSync(resolve(tmpdir(), 'dsh-plugin-manager-typert-'))
const pkg = resolve(staging, 'packages/manager')
...
try {
  ...
  writeFileSync(resolve(protocol, 'tsconfig.json'), JSON.stringify({
    compilerOptions: { target: 'ES2024', module: 'NodeNext', moduleResolution: 'NodeNext', strict: true, noEmit: true, skipLibCheck: true },
    include: ['src'],
  }))
  writeFileSync(resolve(staging, 'tsconfig.host.json'), JSON.stringify({
    ...
    files: [],
    references: [{ path: './packages/manager/tsconfig.build.json' }, { path: './packages/typert-protocol/tsconfig.json' }],
  }))
  const analyzer = new WorkspaceAnalyzer({ root: staging, faces: ['host'], packages: ['dsh-plugin-manager'], checkDiagnostics: true })
  analyzer.discoverPackages()
  const face = analyzer.analyze().faces.find(candidate => candidate.face === 'host')
  const model = face?.packages.find(candidate => candidate.name === 'dsh-plugin-manager')
  if (face === undefined || model === undefined) throw new Error('Typert could not model dsh-plugin-manager')
  const emitted = new FaceModelEmitter(face).emit(model.name)
  if (emitted.remote === undefined) throw new Error('Typert found no Plugin Manager Remote methods')
