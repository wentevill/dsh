    ...
    for (const field of ['dependencies', 'optionalDependencies']) {
        ...
        for (const spec of Object.values(dependencies)) {
            if (typeof spec !== 'string')
                throw managerError('PACKAGE_INVALID', `${field} values must be strings`);
...
function requiredEntrypoints(manifest) {
    const paths = [];
    if (manifest.main !== undefined)
        paths.push(normalizePackagePath(manifest.main, 'main'));
...
export async function inspectPluginArchive(path) {
        ...
        unpack.on('entry', (header, stream, next) => {
            void (async () => {
                ...
                if (entries > MAX_ENTRIES)
                    throw managerError('ARCHIVE_INVALID', 'archive has too many entries');
                ...
                if (names.has(name))
                    throw managerError('ARCHIVE_UNSAFE_ENTRY', 'archive contains a duplicate entry');
                names.add(name);
                const chunks = [];
                let entryBytes = 0;
    ...
    if (manifestBytes === undefined)
        throw managerError('PACKAGE_INCOMPLETE', 'package.json is missing');
    let manifest;
    try {
